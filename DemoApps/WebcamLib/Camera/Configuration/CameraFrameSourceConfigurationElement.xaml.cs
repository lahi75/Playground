...
using Phoebit.Shared.Extensions;

namespace Phoebit.Vision.Camera.Configuration
{
    /// <summary>
    ...
    public partial class CameraFrameSourceConfigurationElement : UserControl
    {
        ...
        public CameraFrameSourceConfigurationElement(CameraFrameSource cameraFrameSource)
        {
            ...
            if (!DesignerProperties.GetIsInDesignMode(this) && _cameraFrameSource != null)
            {
                ...
                camSelectionBox.ItemsSource = cameras;
                camSelectionBox.SelectedItem = _cameraFrameSource.Camera;
            }
        }

        private readonly object _frameSync = new object();
        private bool _frameWaiting = false;
        private void NewFrame(Phoebit.Vision.Contracts.IFrameSource frameSource, Phoebit.Vision.Contracts.Frame frame, double fps)
        {
        ...
        private void camSelectionBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            _cameraFrameSource.Camera = camSelectionBox.SelectedItem as Camera;
            panelCameraInfo.Visibility = camSelectionBox.SelectedItem != null
                                                ? Visibility.Visible
