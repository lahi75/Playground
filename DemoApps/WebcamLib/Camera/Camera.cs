...
using Phoebit.WebcamGrabber;

namespace Phoebit.Vision.Camera
{
    /// <summary>
    /// Represents a camera in use by the Phoebit system
    /// </summary>
    public class Camera : IDisposable
    {
        ...
        /// </summary>
        private double CameraAspectRatio
        {
            get
            {
                if( _camHeight > 0 )
                    return _camWidth / (double)_camHeight;
                return 1;
            }
        }
        ...
        /// gets the aspect ratio of the camera
        /// </summary>
        private double CaptureAspectRatio
        {
            get
            {
                if (_height > 0)
        ...
        private int _camWidth = 320;
        private int _camHeight = 240;

        internal void StartCapture()
        {
            _cameraMethods.StartCamera(_index, ref _camWidth, ref _camHeight);
        }
...
private void CaptureCallbackProc(int dataSize, byte[] data)
{
    ...
    var b = new Bitmap(_camWidth, _camHeight, -stride, PixelFormat.Format24bppRgb, (IntPtr)scan0);
    b.RotateFlip(_rotateFlip);

    // readjust capture aspect ratio to maintain the aspect ratio
    if (CameraAspectRatio != CaptureAspectRatio)
    {
        _height = (int)(_width / CameraAspectRatio);
    }
